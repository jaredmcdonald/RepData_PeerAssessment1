...
Our dataset has weekdays in them (numbers 1-7, corresponding to Sunday through Saturday), so we need to start by subsetting the data:

```{r}
weekend_data <- filter(na_inputted_activity, weekday == 1 | weekday == 7)
weekday_data <- filter(na_inputted_activity, weekday != 1 & weekday != 7)
```

Now, in the same way we did previously, group by interval, and summarize the mean
